...
<div class="relative">
    <!-- Vertical line -->
    <div class={`absolute left-4 inset-y-0 w-0.5 ${lineColor}`}></div>
    
    <!-- Timeline phases -->
    {#each phases as phase, index}
        <div class="relative pl-10 pb-8">
            <!-- Timeline node -->
            <div class={`absolute left-2 -translate-x-1/2 w-6 h-6 rounded-full ${nodeColor} ${nodeTextColor} flex items-center justify-center font-bold text-sm`}>
                {phase.id}
            </div>
            ...
            <div class="bg-white border rounded-lg p-4 shadow-sm">
                ...
                {#if index < phases.length - 1}
                    <div class="mt-4 pt-4 border-t border-dashed border-indigo-200">
                        <div class="flex items-center gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-indigo-600"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg>
                            <p class="text-sm text-slate-600">
                                Transición a <span class="font-medium">{phases[index + 1].name}</span>
                            </p>
                        </div>
                    </div>
                {/if}
