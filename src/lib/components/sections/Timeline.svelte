<script lang="ts">
  ...
  import type { TimelineContent } from '$lib/types'; // Removed TimelinePhase as it's used internally
          ...
          <div class="grid md:grid-cols-2 gap-4 mt-4">
              ...
              <ul class="space-y-1">
                {#each phase.tasks as task}
                  <li class="flex items-start gap-2">
                    ...
                    {:else}
                        <div class="w-2 h-2 rounded-full bg-indigo-500 mt-2"></div> <!-- Fallback for generic bullet -->
                    {/if}
            ...
            <div>
              <h4 class="font-medium text-slate-800 mb-2 flex items-center gap-2">
                ...
                {:else}
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-indigo-600">
                    <path d="M2 15s1.1-1.1 3.5-1.5C8.3 13 9.5 12 9.5 12s1.1-1.1 3.5-1.5"></path>
                    <path d="M19.3 6.8a23 23 0 0 1-3 10.3c-.3.5-1 1.1-1.4 1.4-.3.3-.6.4-1 .5-.5 0-1.3-.2-1.9-1.2-.6-1-4-7.6-4.8-9.1-.2-.4 0-.9.3-1.2.2-.2.4-.3.6-.3h.5c.7.1 2 .5 4.5 4.2.1.2.2.2.3 0 .3-.7 1.1-2.9 1.3-3.7A5 5 0 0 0 15 6c-.2-1.6-1-2-1.7-2.1a2 2 0 0 0-.5 0"></path>
                    <path d="M10.2 10.8c0-1 0-1.5-.5-2 4 1 8.7.8 11.7-1.4a17 17 0 0 1-7 8.4c-2 1-4 2.2-6.2 2.7-3 .7-6.1-.7-7.6-3.1a7.5 7.5 0 0 1-.6-8.5c1.4-2.1 4-3.3 6.4-3a8 8 0 0 1 5.3 2.5c0 .3 0-.8 0 0"></path>
                    <path d="M19 6.6a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3Z"></path>
                  </svg>
                {/if}
  ...
  <div class="bg-indigo-50 rounded-lg p-4">
    <h3 class="font-semibold text-lg mb-3 text-slate-800 flex items-center gap-2">
      {#if content.summaryTitleIcon?.component}
          {@const SummaryTitleIconComponent = content.summaryTitleIcon.component}
          <SummaryTitleIconComponent class={content.summaryTitleIcon.className || 'h-5 w-5 text-indigo-600'} />
      {:else}
